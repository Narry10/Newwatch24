 ...
 */
/**
 * Upload ảnh lên API route /api/images/upload (proxy Google Drive)
 * - Theo dõi progress bằng XHR
 * - Hỗ trợ AbortSignal
 * - Trả về URL public của ảnh từ Google Drive
 */
export const handleImageUpload = async (file, onProgress, abortSignal) => {
  ...
  try {
    ...
    console.error("XHR upload failed, trying fetch fallback:", err);
...
/**
 * Helper: Upload bằng XHR để bắt được onprogress
 */
function uploadViaXHR(url, formData, onProgress, abortSignal) {
